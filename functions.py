...
"extra"]
    ...
    profiles = pd.read_csv(path, sep='\t', header=None, names=profile_columns, usecols=['user_id', 'public', 'completion_percentage', 'gender', 'age'], index_col=None, nrows=5)
    ...
    profiles.drop(['completion_percentage', 'public'], inplace=True)
...
def load_data(edgelist_path, node_path):
    ...
    the edgelist and public nodes as a tuple of dataframes.
    """

    edgelist = preparing_edges(edgelist_path)
    
    # getting nodes that are in the reduced edgelist
    nodes_with_edges = pd.Index(edgelist.source).intersection(pd.Index(edgelist.target))
    
    nodes = preparing_nodes
